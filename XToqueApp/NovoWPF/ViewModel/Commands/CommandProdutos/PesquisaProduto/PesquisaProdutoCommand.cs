...
namespace NovoWPF.ViewModel.Commands.CommandProdutos.PesquisaProduto
{
    public class PesquisaProdutoCommand : CommandBase
    {
        ...
        {
            var dadosGrid = Produtos.Where(p => p.NomeProduto.Contains(ProdutoView.txtBoxPesquisaProduto.Text) || p.Codigo.Contains(ProdutoView.txtBoxPesquisaProduto.Text)).ToList();
